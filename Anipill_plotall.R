...
#svg(filename = "./graphix/248-Tb-exp.svg", width =17, height = 5)
#print(p)
#dev.off()
...
#With lines as photperiod, lookm in wide form
p <- ggplot(subset(allcoreT, ID %in% c("H248")), aes(x=datetime, y=ap_t))+
...
p <- ggplot(subset(allcoreT, ID %in% c("H216")), aes(x=datetime, y=ap_t, color=ID))+
  scale_x_datetime( date_breaks = "1 week", date_labels = "%W")+
  geom_line(alpha=0.9, linewidth = 1)+theme_classic()
#geom_line(data = Tamb, aes(y=rt), color='steelblue', alpha=0.7)+
#labs(x="", y= "Core Body Temperature (C)")+
#geom_vline(xintercept = strptime(c("12/08/21 16:03"), format = "%d/%m/%y %H:%M", tz="GMT"), alpha=1, size=0.5, color="red")+
#geom_vline(xintercept = strptime(c("13/08/21 13:04"), format = "%d/%m/%y %H:%M", tz="GMT"), alpha=1, size=0.5, color="red")+
#geom_vline(xintercept = strptime(c("11/08/21 13:46"), format = "%d/%m/%y %H:%M", tz="GMT"), alpha=1, size=0.5, color="red")+
#geom_vline(xintercept = strptime(c("11/08/21 16:52"), format = "%d/%m/%y %H:%M", tz="GMT"), alpha=1, size=0.5, color="red")
